...
std::vector<Device> Lpf2Hub::getConnectedDevices()
{
    std::vector<Device> devices;
    for (int i = 0; i < numberOfConnectedDevices; i++)
    {
        devices.push_back(connectedDevices[i]);
    }
    return devices;
}
...
void Lpf2Hub::parseDeviceInfo(uint8_t *pData)
{
    ...
    {
        _hubPropertyChangeCallback(this, (Lpf2HubPropertyReference)pData[3], pData);
        return;
    }

    if (pData[3] == (byte)Lpf2HubPropertyReference::ADVERTISING_NAME)
    {
        log_d("advertising name: %s", parseHubAdvertisingName(pData).c_str());
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::BUTTON)
    {
        log_d("hub button state: %x", parseHubButton(pData));
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::FW_VERSION)
    {
        Lpf2Version version = parseVersion(pData);
        log_d("version: %d-%d-%d (%d)", version.Major, version.Minor, version.Bugfix, version.Build);
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::HW_VERSION)
    {
        Lpf2Version version = parseVersion(pData);
        log_d("version: %d-%d-%d (%d)", version.Major, version.Minor, version.Bugfix, version.Build);
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::RSSI)
    {
        log_d("rssi: ", parseRssi(pData));
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::BATTERY_VOLTAGE)
    {
        log_d("battery level: %d %", parseBatteryLevel(pData));
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::BATTERY_TYPE)
    {
        log_d("battery type: %d", parseBatteryType(pData));
        return;
    }
    else if (pData[3] == (byte)Lpf2HubPropertyReference::SYSTEM_TYPE_ID)
    {
 ...
 * @return Pant type
 */
Lpf2MarioPant Lpf2Hub::parseMarioPant(uint8_t *pData)
{
    int value = LegoinoCommon::ReadInt8(pData, 4);
    log_d("Mario Pant: %d", value);
    return (Lpf2MarioPant)value;
}
 ...
 * @return Gesture
 */
Lpf2MarioGesture Lpf2Hub::parseMarioGesture(uint8_t *pData)
{
    int value = LegoinoCommon::ReadInt16LE(pData, 4);
    log_d("Mario Gesture: %d", value);
    return (Lpf2MarioGesture)value;
}
 ...
 * @return MarioBarcode
 */
Lpf2MarioBarcode Lpf2Hub::parseMarioBarcode(uint8_t *pData)
{
    int value = LegoinoCommon::ReadInt16LE(pData, 4);
    log_d("Mario Barcode: %d", value);
    return Lpf2MarioBarcode(value);
}
 ...
 * @return MarioColor
 */
Lpf2MarioColor Lpf2Hub::parseMarioColor(uint8_t *pData)
{
    int value = LegoinoCommon::ReadInt16LE(pData, 6);
    log_d("Mario Color: %d", value);
    return (Lpf2MarioColor)value;
}
 ...
 * @return button state
 */
Lpf2ButtonState Lpf2Hub::parseRemoteButton(uint8_t *pData)
{
    int buttonState = pData[4];
    log_d("remote button state: %x", buttonState);
    return (Lpf2ButtonState)buttonState;
}
 ...
 * @return button state
 */
Lpf2ButtonState Lpf2Hub::parseHubButton(uint8_t *pData)
{
 ...
 */
Lpf2Version Lpf2Hub::parseVersion(uint8_t *pData)
{
    Lpf2Version version;
...
byte Lpf2Hub::getModeForDeviceType(byte deviceType)
{
    switch (deviceType)
    {
    case (byte)Lpf2DeviceType::SIMPLE_MEDIUM_LINEAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::TRAIN_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::MEDIUM_LINEAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::MOVE_HUB_MEDIUM_LINEAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::COLOR_DISTANCE_SENSOR:
        return 0x08;
    case (byte)Lpf2DeviceType::MOVE_HUB_TILT_SENSOR:
        return 0x00;
    case (byte)Lpf2DeviceType::TECHNIC_MEDIUM_ANGULAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::TECHNIC_LARGE_ANGULAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::TECHNIC_LARGE_LINEAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::TECHNIC_XLARGE_LINEAR_MOTOR:
        return (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM;
    case (byte)Lpf2DeviceType::MARIO_HUB_GESTURE_SENSOR:
        return 0x01;
    default:
        return 0x00;
    }
...
void Lpf2Hub::parseSensorMessage(uint8_t *pData)
{
    ...
    {
        connectedDevices[deviceIndex].Callback(this, pData[3], (Lpf2DeviceType)deviceType, pData);
        return;
    }

    if (deviceType == (byte)Lpf2DeviceType::CURRENT_SENSOR)
    {
        parseCurrentSensor(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::VOLTAGE_SENSOR)
    {
        parseVoltageSensor(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::MEDIUM_LINEAR_MOTOR || deviceType == (byte)Lpf2DeviceType::MOVE_HUB_MEDIUM_LINEAR_MOTOR)
    {
        parseTachoMotor(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::DUPLO_TRAIN_BASE_SPEEDOMETER)
    {
        parseSpeedometer(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::DUPLO_TRAIN_BASE_COLOR_SENSOR)
    {
        parseColor(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::COLOR_DISTANCE_SENSOR)
    {
        parseDistance(pData);
        parseColor(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::MOVE_HUB_TILT_SENSOR)
    {
        parseBoostTiltSensorX(pData);
        parseBoostTiltSensorY(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::TECHNIC_MEDIUM_HUB_TILT_SENSOR)
    {
    ...
    else if (deviceType == (byte)Lpf2DeviceType::REMOTE_CONTROL_BUTTON)
    {
        int port = pData[3];
        parseRemoteButton(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::MARIO_HUB_GESTURE_SENSOR)
    {
        int port = pData[3];
        parseMarioGesture(pData);
        return;
    }
    else if (deviceType == (byte)Lpf2DeviceType::MARIO_HUB_BARCODE_SENSOR)
    {
        int port = pData[3];
    ...
    else if (deviceType == (byte)Lpf2DeviceType::MARIO_HUB_PANT_SENSOR)
    {
        int port = pData[3];
    ...
    bool isNotify)
{
    ...
    switch (pData[2])
    {
    case (byte)Lpf2MessageType::HUB_PROPERTIES:
    {
        parseDeviceInfo(pData);
        break;
    }
    case (byte)Lpf2MessageType::HUB_ATTACHED_IO:
    {
        parsePortMessage(pData);
        break;
    }
    case (byte)Lpf2MessageType::PORT_VALUE_SINGLE:
    {
        parseSensorMessage(pData);
        break;
    }
    case (byte)Lpf2MessageType::PORT_OUTPUT_COMMAND_FEEDBACK:
    {
 ...
 */
void Lpf2Hub::setLedColor(Lpf2Color color)
{
    byte port = getPortForDeviceType((byte)Lpf2DeviceType::HUB_LED);
 ...
 */
void Lpf2Hub::activateHubPropertyUpdate(Lpf2HubPropertyReference hubProperty, HubPropertyChangeCallback hubPropertyChangeCallback)
{
    ...
    // Activate reports
    byte notifyPropertyCommand[3] = {0x01, (byte)hubProperty, (byte)Lpf2HubPropertyOperation::ENABLE_UPDATES_DOWNSTREAM};
 ...
 * @param [in] optional callback function which will be called if a value has changed
 */
void Lpf2Hub::requestHubPropertyUpdate(Lpf2HubPropertyReference hubProperty, HubPropertyChangeCallback hubPropertyChangeCallback)
{
    if (hubPropertyChangeCallback != nullptr)
    {
        _hubPropertyChangeCallback = hubPropertyChangeCallback;
    ...
    // Activate reports
    byte notifyPropertyCommand[3] = {0x01, (byte)hubProperty, (byte)Lpf2HubPropertyOperation::REQUEST_UPDATE_DOWNSTREAM};
    WriteValue(notifyPropertyCommand, 3);
}
 ...
 * @param [in] hubProperty for which updates should be activated
 */
void Lpf2Hub::deactivateHubPropertyUpdate(Lpf2HubPropertyReference hubProperty)
{

    // Activate reports
    byte notifyPropertyCommand[3] = {0x01, (byte)hubProperty, (byte)Lpf2HubPropertyOperation::DISABLE_UPDATES_DOWNSTREAM};
    WriteValue(notifyPropertyCommand, 3);
}
 ...
 * @param [in] speed Speed of the Motor -100..0..100 negative values will reverse the rotation
 */
void Lpf2Hub::setTachoMotorSpeed(byte port, int speed, byte maxPower, Lpf2BrakingStyle brakingStyle)
{
 ...
 */
void Lpf2Hub::setTachoMotorSpeedForTime(byte port, int speed, int16_t time = 0, byte maxPower, Lpf2BrakingStyle brakingStyle)
{
    //Use acc and dec profile (0x03 last two bits set)
    byte *timeBytes = LegoinoCommon::Int16ToByteArray(time);
 ...
 * @param [in] brakingStyle Braking style how the motor will stop. Brake(default), Float, Hold are available
 */
void Lpf2Hub::setTachoMotorSpeedForDegrees(byte port, int speed, int32_t degrees, byte maxPower, Lpf2BrakingStyle brakingStyle)
{
 ...
 * @param [in] brakingStyle Braking style how the motor will stop. Brake(default), Float, Hold are available
 */
void Lpf2Hub::setAbsoluteMotorPosition(byte port, int speed, int32_t position, byte maxPower, Lpf2BrakingStyle brakingStyle)
{
 ...
 * @param [in] brakingStyle Braking style how the motor will stop. Brake(default), Float, Hold are available
 */
void Lpf2Hub::setTachoMotorSpeedsForDegrees(int speedLeft, int speedRight, int32_t degrees, byte maxPower, Lpf2BrakingStyle brakingStyle)
{
    byte *degreeBytes = LegoinoCommon::Int32ToByteArray(degrees);
    byte port = (byte)Lpf2MoveHubPort::AB;
    //Use acc and dec profile (0x03 last two bits set)
